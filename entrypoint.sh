...
sed -i /etc/proftpd/proftpd.conf -e 's/# DefaultRoot/DefaultRoot/' -e "s/Port\s*21/Port ${FTP_PORT_CONTROL}/" -e 's#/var/log/.*#/proc/self/fd/1#'
#-e 's#TransferLog.*#TransferLog /proc/self/fd/1#' -e 's#SystemLog.*#SystemLog /proc/self/fd/1#'
