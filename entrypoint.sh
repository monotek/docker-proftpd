...
sed -i /etc/proftpd/proftpd.conf -e 's/# DefaultRoot/DefaultRoot/' -e "s/Port\s*21/Port ${FTP_PORT_CONTROL}/" -e 's#TransferLog.*#TransferLog /dev/stdout#' -e 's#SystemLog.*#SystemLog /dev/stdout#'
